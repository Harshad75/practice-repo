...
environment {
  ...
  stages {
    ...
    stage('SonarQube analysis') {
      environment {
       scannerHome = tool 'sonar-practice-scanner'
      }
      steps{ 
        withSonarQubeEnv('sonar-practice-server') { // If you have configured more than one global server connection, you can specify its name
